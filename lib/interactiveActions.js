...
module.exports = {
	...
	interactiveAddGroup:function(callback) {
		var questions = [
	...
	interactiveAddGroupUsers: function(callback) {
			...
			methods.getGroups(
				...
				function(error, results) {
					if (!error) {
						...
						inquirer.prompt(
							{ name: 'group', message: 'Which group?', type: 'list', choices: groupNames },
							function(answers) {

								groupId = _.find(groups, function(group) {
									return answers.group === group.nameCurrentValue;
								}).groupId;
			...
			methods.getCompanyUsers(
				...
				function(error, results) {
					if (!error) {
						...
						userScreenNames = _.map(users, function(user) {
							return user.screenName;
						});

						userScreenNames = _.reject(userScreenNames, function(userScreenName) {
							return _.contains(currentGroupUsers, userScreenName);
						});
		...
		function ask(userScreenNames, currentGroupUsers) {
			inquirer.prompt(
				{ name: 'users', message: 'Which users?', type: 'checkbox', choices: userScreenNames },
				function(answers) {
					users = _.filter(users, function(user) {
						return _.contains(answers.users, user.screenName);
					});

					userIds = _.map(users, function(user) {
						return user.userId;
					})
