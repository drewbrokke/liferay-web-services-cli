...
module.exports = {
	...
	getCommands: function(type) {
		var commands = {};
		var commandDir = path.join(__dirname, '../commands', type);
		var commandFileList = fs.readdirSync(commandDir);

		function indexCommand(fileName) {
			var command = require(path.join(commandDir, fileName));

			var baseName = type + '-' + fileName.replace('.js', '');

			var commandName = _.camelCase(baseName);
